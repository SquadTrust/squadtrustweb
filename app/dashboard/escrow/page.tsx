...
import { useEscrowTransactions, useCreateEscrow } from '../../../lib/hooks/useEscrow';
...
export default function EscrowPage() {
  ...
  const handleCreateSubmit = async (data: { customerPhone: string, description: string, amountKobo: number, deliveryMethod: string }) => {
    ...
    >
      <motion.div variants={itemVariants} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Create secure payment links and track funds.</p>
        </div>
        <button 
          onClick={() => { setIsFormOpen(true); setCreatedLink(null); }}
          ...
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full relative max-h-[90vh] overflow-y-auto">
            ...
            {createdLink ? (
              <div className="p-8 text-center space-y-4">
                ...
                <h3 className="text-xl font-bold text-gray-900">Escrow Created!</h3>
                <p className="text-sm text-gray-600">Share this link with your buyer:</p>
              ...
              <div className="p-2">
                <CreateEscrowForm onSubmit={handleCreateSubmit} isLoading={createEscrow.isPending} />
              </div>
            )}
          </div>
        </div>
      ...
      <motion.div variants={itemVariants} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden w-full">
        <div className="px-5 md:px-6 py-4 border-b border-gray-100 bg-gray-50/50">
          <h2 className="text-sm font-semibold text-gray-900 uppercase tracking-wider">All Transactions</h2>
        </div>
